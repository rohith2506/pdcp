    ...
    public static void runParallelFuzzyBarrier(final int iterations,
        ...
        for (int ii = 0; ii < tasks; ii++) {
            ...
            threads[ii] = new Thread(() -> {
                ...
                for (int iter = 0; iter < iterations; iter++) {
                    final int left = i * (n / tasks) + 1;
                    threadPrivateMyNew[left] = (threadPrivateMyVal[left-1] + threadPrivateMyVal[left+1]) * 1.0 / 2.0;

                    final int right = (i + 1) * (n / tasks);
                    threadPrivateMyNew[right] = (threadPrivateMyVal[right-1] + threadPrivateMyVal[right+1]) * 1.0 / 2.0;
